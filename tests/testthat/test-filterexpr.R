...
test_that("filter_matrix throws an error if norm_counts is not numeric", {
  nm <- named_nm_matrix(10, 15, min = 1000, max = 1500)
  expect_error(filter_matrix(counts = nm, norm_counts = letters[1:5]), "norm_counts must be a numeric matrix")
  expect_error(filter_matrix(counts = nm, norm_counts = matrix(rep(TRUE, 12), nrow = 3)), "norm_counts must be a numeric matrix")
})

test_that("filter_matrix throws an error if genes_keep is not character", {
  nm <- named_nm_matrix(10, 15, min = 1000, max = 1500)
  expect_error(filter_matrix(counts = nm, genes_keep = runif(10)), "genes_keep must be a character vector")
  expect_error(filter_matrix(counts = nm, genes_keep = rep(TRUE, 12)), "genes_keep must be a character vector")
})
...
test_that("filter_matrix throws warning when missing BCCB genes found", {
  ...
  with_mocked_bindings({
    expect_warning(filter_matrix(nm), paste0(n, "/", length(bccb_genes), " BCCB genes were not found in the matrix. Are you using gene symbols?"))
...
test_that("filter_matrix throws warning when missing genes found", {
  ...
  egm <- mock(sample(rownames(nm), size = 50))
  with_mocked_bindings({
    expect_warning(filter_matrix(nm, genes_keep = genes),
