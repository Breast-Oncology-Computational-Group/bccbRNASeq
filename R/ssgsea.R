...
##' }
#' @export
ss_gsea <- function(norm_counts, set_list, seed = 37, cores = getOption("mc.cores", 2L)) {
  ...
  fgsea_list <- parallel::mclapply(colnames(norm_counts), function (sample) {
      ...
      set.seed(seed)
      results <- fgsea::fgsea(pathways = set_list, stats = vstat,
                   eps = 1e-8, minSize  = 10, maxSize  = 1000, nPermSimple = 10000, scoreType="pos", nproc = 1)
      ### the nproc=1 is important according to: https://github.com/ctlab/fgsea/issues/12
...
bccb_expression_signatures <- function(norm_counts, signature_set = "all", signature_names = NULL,
  ...
  rlang::arg_match(signature_set, values = c("hallmark", "c2cgp_breast", "all"))
  ...
  if(!is.null(signature_names)) {
    ...
    signature_results <- lapply(names(bccb_signatures), function(ss) {
      results <- ss_gsea(norm_counts, bccb_signatures[[ss]], seed = seed, cores = cores)
      results <- results |>
